...
from .models import User,Profile,ParaProfile,CustomerProfile
from .serializers import (SimpleUserSerializer,UserSerializer,ParaProfileListSerializer,
                           ParaProfileSerializer,CustomerProfileSerializer,CustomerProfileListSerializer)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_user_info(self,request):
        serializer = SimpleUserSerializer(request.user,many=False,context={'request':request})
...
class ParaProfileViewSet(viewsets.ModelViewSet):
    queryset = ParaProfile.objects.all()
    serializer_class = ParaProfileSerializer
    def list(self,request):
        serializer = ParaProfileListSerializer(self.queryset,many=True)
        return Response({'para_list':serializer.data},status=status.HTTP_200_OK)
...
class CustomerProfileViewSet(viewsets.ModelViewSet):
    ...
    def list(self,request):
        ...
        serializer = CustomerProfileListSerializer(customers,many=True,context={'requets':request})
        return Response({'customer_list':serializer.data},status=status.HTTP_200_OK)
